if Meteor.isClient
  ...
  Template.socialContent.helpers
    ...
    feedscount:()->
      Feeds.find({followby:Meteor.userId(),checked:false, eventType: {$ne: 'share'}, createdAt:{$gt:new Date((new Date()).getTime() - 7 * 24 * 3600 * 1000)}},{sort: {createdAt: -1}, limit:20}).count()
    haveFeeds:->
      if Feeds.find({followby:Meteor.userId(),checked:false, eventType: {$ne: 'share'}, createdAt:{$gt:new Date((new Date()).getTime() - 7 * 24 * 3600 * 1000)}},{sort: {createdAt: -1}, limit:20}).count()>0
        true
      else
        false
    haveNewFriends: ->
