...
Readability = (function(){
  var readability = {
     ...
     * @return element
     **/
    init: function(documentBodyElement, preserveUnlikelyCandidates) {
        preserveUnlikelyCandidates = (typeof preserveUnlikelyCandidates == 'undefined') ? false : preserveUnlikelyCandidates;
        readability.bodyCache = documentBodyElement.innerHTML;
        readability.documentBodyElement.innerHTML = documentBodyElement.innerHTML;
        ...
        **/
        if(readability.getInnerText(articleContent, false) == "")
        {
            if(!preserveUnlikelyCandidates) {
                readability.documentBodyElement.innerHTML = readability.bodyCache;
        ...
        readability.documentBodyElement.className = readability.readStyle;
        ...
        readability.documentBodyElement.insertBefore(overlay, readability.documentBodyElement.firstChild);

        if(readability.frameHack)
        {
            var readOverlay = $(readability.documentBodyElement).find('#readOverlay')[0];
            readOverlay.style.height = '100%';
            readOverlay.style.overflow = 'auto';
        }
      
        return readability.documentBodyElement;
    },
    ...
    prepDocument: function () {
        ...
        readability.documentBodyElement.innerHTML = readability.documentBodyElement.innerHTML.replace(readability.regexps.replaceBrsRe, '</p><p>').replace(readability.regexps.replaceFontsRe, '<$1span>')
    },
    ...
    **/
    grabArticle: function (preserveUnlikelyCandidates) {
        ...
        **/
        for(var nodeIndex = 0; (node = readability.documentBodyElement.getElementsByTagName('*')[nodeIndex]); nodeIndex++)
        {
        ...
        **/
        var allParagraphs = readability.documentBodyElement.getElementsByTagName("p");
        var candidates    = [];
    ...
    **/
    ...
    emailBox: function () {
        ...
        emailContainer.innerHTML = '<iframe src="'+readability.emailSrc + '?pageUrl='+escape(window.location)+'&pageTitle='+escape('')+'" scrolling="no" onload="readability.removeFrame()" style="width:500px; height: 490px; border: 0;"></iframe>';

        readability.documentBodyElement.appendChild(emailContainer);			
    },
     ...
     **/
    kindleBox: function () {
        var kindleContainer = $(readability.documentBodyElement).find('#kindle-container')[0];
        ...
        readability.documentBodyElement.appendChild(kindleContainer);
        ...
        var formHtml =  '<form id="readabilityKindleForm" style="display: none;" target="readabilityKindleIframe" method="post" action="' + readability.kindleSrc + '">\
                             <input type="hidden" name="bodyContent" id="bodyContent" value="' + readability.htmlspecialchars($(readability.documentBodyElement).find('#readability-content')[0].innerHTML) + '" />\
                             <input type="hidden" name="pageUrl" id="pageUrl" value="' + readability.htmlspecialchars('') + '" />\
                             <input type="hidden" name="pageTitle" id="pageUrl" value="' + readability.htmlspecialchars('') + '" />\
                         </form>';

        readability.documentBodyElement.innerHTML += formHtml;
        $(readability.documentBodyElement).find('#readabilityKindleForm')[0].submit();
    },
    ...
    removeFrame: function () {
        ...
        {
            var emailContainer = $(readability.documentBodyElement).find('#email-container')[0];
            if (null !== emailContainer) {
  ...
  Readability.prototype.execute = function(documentBodyElement){return readability.init(documentBodyElement);}
  return Readability;
})();
