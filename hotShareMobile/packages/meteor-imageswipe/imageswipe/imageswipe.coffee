...
class ImageSwipe
  ...
  setInitialPage: (name) ->
    ...
    console.log "set initial page: " + name
    for n in @templates
  ...
  click: (template, selector, handler) ->
    ...
    eventMap[mouseup] = (e,t) ->
      if ImagesSwiper.shouldControl() and not ImagesSwiper.t.touchDown
        eventPrint "mouseup control"
        handler.bind(@)(e,t)

    eventMap[touchend] = (e,t) ->
      if e.currentTarget is ImagesSwiper.element and ImagesSwiper.shouldControl()
...
Template.imageswipe.helpers
  pageNames: -> _.map @ImagesSwiper?.templates, (name) -> {name: name}


Template.imageswipe.rendered = ->
  # check that templates is passed
  if not @data.ImagesSwiper
    console.log("ERROR: must pass a Swipe object.")
  else
    # Bind the Swiper to this template and the template to the swiper
    @ImagesSwiper = @data.ImagesSwiper
    @ImagesSwiper.setTemplate(@)
...
Template.imageswipe.events
  ...
  'mouseup .pages': (e,t) ->
    ...
    if t.mouseDown
      ...
      if swipeControlCSS and (e.target is t.ImagesSwiper.element) and t.ImagesSwiper.shouldControl()
        t.velX = 0
        t.startX = 0
        t.mouseX = 0
        t.changeX = 0
      ...
      if index is -1
        t.ImagesSwiper.moveRight()
      else if index is 1
        t.ImagesSwiper.moveLeft()
      else
        t.ImagesSwiper.animateBack()
