package com.example.gangzhang.videotest1.rtspclient.RtspClinet.Stream;
...
public abstract class RtpStream {
    ...
    private void unpackData() {
        ...
        tmpStreampack.data = new byte[buffer.length-28];

        System.arraycopy(buffer, 28, tmpStreampack.data, 0, buffer.length - 28);
        if(oldSeqNum==-1) oldSeqNum = tmpStreampack.sequenceNumber;

        if (tmpStreampack.sequenceNumber - oldSeqNum>1){
            Log.e("myapp","RTP lost packet:"+oldSeqNum+"..."+tmpStreampack.sequenceNumber);
        }
