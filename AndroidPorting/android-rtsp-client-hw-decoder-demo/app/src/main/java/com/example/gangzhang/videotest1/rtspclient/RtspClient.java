package com.example.gangzhang.videotest1.rtspclient;
...
import  com.example.gangzhang.videotest1.rtspclient.RtspClinet.Video.H264Stream;
import com.example.gangzhang.videotest1.rtspclient.RtspClinet.Socket.RtpSocket;
...
public class RtspClient {
    ...
    public void setSurfaceView( TextureView s ) {
        mSurfaceView = s;
    }
    ...
    private void ClientConfig(String host, String address, int port) {
        ...
        if(authorName == null && authorPassword == null) {
            ...
            authorBase64 = null;//Base64.encodeToString((authorName+":"+authorPassword).getBytes(),Base64.DEFAULT);
        }
    ...
    private void sendAuthHandshake() throws IOException {
        String request = "OPTIONS rtsp://" + mParams.address + " RTSP/1.0\r\n" + addHeaders();
        Log.d(tag, request.substring(0, request.indexOf("\r\n")));
        mOutputStream.write(request.getBytes("UTF-8"));
        Response response=Response.parseHandshake(mBufferreader);
        request = "OPTIONS rtsp://" + mParams.address + " RTSP/1.0\r\n" +
                addHeadersWithDigest(getDigestAuthReponse("OPTIONS",mParams.address,authorName,authorPassword,
                        response.digestRealm,response.digestNonce));
    ...
    private String addHeadersWithDigest(String digestAuthString) {
        return "CSeq: " + (++CSeq) + "\r\n"
                + digestAuthString
    ...
    private String getDigestAuthReponse(String method, String digestUri, String userName,String passwd, String digestRealm,
                               String digestNonce) throws IOException
    {
        try
        {
            ...
            Log.d(tag,"username="+userName+"realm="+digestRealm+", nonce="+digestNonce+
                            ", uri="+digestUri+", response="+response);

            responseString+="Authorization: Digest";
            responseString+=" username=\"" + userName + "\"";
            responseString+=", realm=\"" + digestRealm + "\"";
            responseString+=", nonce=\"" + digestNonce + "\"";
            responseString+=", uri=\"" + digestUri + "\"";
        ...
        {
            Log.e(tag,"Cannot generate Digest Auth header", e);
            return "";
        }
    }
    ...
    static class Response {
        ...
        public String digestNonce;
        public boolean needAuth;

        public static HashMap<String,String> headers = new HashMap<String,String>();
        public static Response parseHandshake(BufferedReader input) throws IOException  {
            Response response = new Response();
            String line;
            Matcher matcher;
            int sdpContentLength = 0;
            //Log.d(tag, "The buffer is: " + input.read());
            if( (line = input.readLine()) == null) throw new IOException("Connection lost");

            //Log.d(tag, "The line.length is: " + line.toString());
            //while ( (line = input.readLine()) != null ) {
            //    Log.d(tag, "The buffer is: " + line);
            //}
            ...
            while ( (line = input.readLine()) != null) {
//                Log.d(tag, "The Describeflag is: " + Describeflag + "...");
                if (line.startsWith("WWW-Authenticate:"))
                {
                    if (line.contains("Digest"))
                    {
                        ...
                        response.digestRealm = line.substring(begin, end);

                        begin = line.indexOf("nonce=");
                        begin = line.indexOf('"', begin)+1;
                        end = line.indexOf('"', begin);
                        response.digestNonce = line.substring(begin, end);
                    }
                    ...
                    Log.d(tag, "Realm: "+response.digestRealm + " digestNonce: " + response.digestNonce);
                    break;
                }
        ...
        public static Response parseResponse(BufferedReader input) throws IOException  {
            ...
            while ( (line = input.readLine()) != null) {
//                Log.d(tag, "The Describeflag is: " + Describeflag + "...");
//                Log.d(tag, "The line.length() is: " + line.length() + "...");
