var timeouts = {};
var addWhatsUpTip = function(userId, deviceId){
  if (!timeouts['_' + userId])
    timeouts['_' + userId] = {userId: userId, deviceId: deviceId, time: new Date().getTime(), count: 0};
  else
...
CreateSatsUpTipTask = function(userId, groupId, deviceId){
  ...
  if (group && group.whats_up_send)
    return addWhatsUpTip(userId, deviceId);
...
Meteor.startup(function(){
  var upsetWhatsUpTip = function(doc){
    if (!doc.whats_up && doc.in_time != 0){
      ...
      if (group && group.whats_up_send)
        return addWhatsUpTip(doc.app_user_id, doc.in_uuid);
    }
